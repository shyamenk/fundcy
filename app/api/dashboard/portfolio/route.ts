...
export async function POST(request: NextRequest) {
  try {
    ...
    if (operationType === "holding") {
      ...
      if (!data.holdingId || !data.amount || !data.frequency || !data.startDate) {
        return NextResponse.json({ error: "Missing required fields for SIP" }, { status: 400 })
      }
      ...
      if (holding.length === 0) {
        return NextResponse.json({ error: "Selected holding not found" }, { status: 400 })
      }
