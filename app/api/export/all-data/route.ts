...
export async function GET() {
  try {
    ...
    const [
      ...
      db.select().from(goalContributions).where(eq(goalContributions.userId, session.user.id)),
      db.select().from(investmentHoldings).where(eq(investmentHoldings.userId, session.user.id)),
      db.select().from(sipInvestments).where(eq(sipInvestments.userId, session.user.id)),
      db.select().from(debitCardExpenses).where(eq(debitCardExpenses.userId, session.user.id)),
    ])
